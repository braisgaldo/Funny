...
class PruebaSalon {
    ...
    private fun juego(): JuegoViewModel =
        JuegoViewModel(
    ...
    @Test
    fun `un mando se conecta al hub y el hub lo apunta como participante`() =
        runTest(dispatcher) {
            ...
            assertTrue(
                vmJuego.estado.participantes.any {
                    it.nombre == "Ana" && it.dispositivo == "mando-1"
                },
    ...
    @Test
    fun `un salon pone la partida en modo individual`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `el hub no admite mas mandos de los que aguanta la topologia`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
            val vmJuego = juego()
            val hub = SalonViewModel()
    ...
    @Test
    fun `la palabra secreta llega solo al movil de quien actua`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
    ...
    @Test
    fun `un juego que no es de actuar no manda contenido privado a nadie`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
            val radioAna = TransporteDeMentira("ana")
            radioHub.enchufar(radioAna)
            ...
            val hayPrivado =
                radioHub.enviados.any { (_, mensaje) ->
                    (mensaje as? es.ghatostudio.funny.dominio.salon.Mensaje.Vista)
                        ?.vista
                        ?.contenidoPrivado
                        ?.isNotEmpty() == true
    ...
    @Test
    fun `un mando no puede tirar el dado en el turno de otro`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
            val radioAna = TransporteDeMentira("ana")
            val radioBea = TransporteDeMentira("bea")
            ...
            mandoIntruso.enviar(
                "hub",
                es.ghatostudio.funny.dominio.salon.Mensaje
                    .Accion(TipoAccion.TIRAR),
            )
            advanceUntilIdle()
    ...
    @Test
    fun `un mando puede renombrarse en cualquier momento`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
            val radioAna = TransporteDeMentira("ana")
            radioHub.enchufar(radioAna)
    ...
    fun `en una ronda de todos se espera a todos y se cuentan bien los aciertos`() =
        runTest(dispatcher) {
            ...
            assertEquals(
                1,
                vmJuego.estado.avanceExtra.size,
                "aciertos: ${vmJuego.estado.avanceExtra}",
            )
        }
    ...
    @Test
    fun `si faltan permisos el salon lo dice y no arranca`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `perder la conexion deja al mando sin vista y sin conectado`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
    ...
    @Test
    fun `si un mando se va el hub lo quita de la lista`() =
        runTest(dispatcher) {
            val radioHub = TransporteDeMentira("hub")
            val radioMando = TransporteDeMentira("mando")
            radioHub.enchufar(radioMando)
    ...
    @Test
    fun `cerrar el salon libera la radio y deja el estado limpio`() =
        runTest(dispatcher) {
            val radio = TransporteDeMentira("hub")
            val salon = SalonViewModel()
            salon.abrirComoHub(radio, "Mesa", juego())
    ...
    @Test
    fun `las mesas encontradas no se duplican`() =
        runTest(dispatcher) {
            val radioMando = TransporteDeMentira("mando")
