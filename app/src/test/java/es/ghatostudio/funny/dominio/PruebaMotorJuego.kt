...
class PruebaMotorJuego {
    ...
    private val contenidoDePrueba = Contenido(
        mimica = List(30) { "mimica $it" },
        dibujo = List(30) { "dibujo $it" },
        eventos = List(20) { EventoCuando("evento $it", 1900 + it) },
        preguntas = List(20) { PreguntaTrivial("pregunta $it", listOf("a", "b", "c", "d"), it % 4) },
    ...
    private fun motor(contenido: Contenido = contenidoDePrueba) = MotorJuego(
        contenido = contenido,
    ...
    fun `el destino nunca pasa de la meta`() {
        ...
        estado = estado.copy(
            participantes = estado.participantes.mapIndexed { i, p ->
    ...
    fun `caer en la meta marca la prueba como final`() {
        ...
        val estado = base.copy(
            participantes = base.participantes.mapIndexed { i, p ->
    ...
    fun `una partida entera termina con un ganador`() {
        val m = motor()
        var estado = m.empezarCarrera(
            EstadoJuego(ajustes = Ajustes(duracion = Duracion.CORTA), participantes = equipos(2)),
            equipos(2)
        )
        var vueltas = 0
        while (estado.pantalla != Pantalla.VICTORIA && vueltas < 500) {
            ...
            estado = if (estado.pantalla == Pantalla.RONDA_TODOS) {
