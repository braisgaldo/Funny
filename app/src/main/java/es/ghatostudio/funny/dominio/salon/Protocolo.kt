 ...
 *
 * Un dispositivo hace de **mesa** (el hub) y los demás de **mando**. El hub es
 * el único que tiene el estado de la partida y el único que aplica las reglas;
 * los mandos solo mandan intenciones («he tocado la opción B») y reciben lo que
 * les toca ver. Esto evita de raíz el problema difícil de verdad de un juego en
 * red —que dos dispositivos crean cosas distintas— y encaja con cómo se juega
 * en una mesa: hay un móvil en el centro y los demás en las manos.
 *
...
sealed interface Mensaje {
    ...
    data class Hola(val nombre: String, val version: Int = VERSION_PROTOCOLO) : Mensaje
    ...
    data class Vista(val vista: VistaDelMando) : Mensaje

    /** Mando → hub: una acción de quien juega. */
    data class Accion(val tipo: TipoAccion, val entero: Int = 0, val texto: String = "") : Mensaje

    /** Hub → todos: el salón se cierra. */
    data object Adios : Mensaje

    /** Mensaje recibido que no se sabe interpretar. Se ignora. */
    data class Desconocido(val tipo: String) : Mensaje
}
...
object Codec {
    ...
    fun aTexto(mensaje: Mensaje): String = when (mensaje) {
        is Mensaje.Hola -> JSONObject()
            .put(TIPO, "hola")
            .put("nombre", mensaje.nombre)
            .put("version", mensaje.version)
            .toString()

        is Mensaje.Salon -> JSONObject()
            .put(TIPO, "salon")
    ...
    fun deTexto(texto: String): Mensaje = runCatching {
        ...
        when (val tipo = o.optString(TIPO)) {
            ...
            "salon" -> {
                ...
                Mensaje.Salon(
                    dispositivos = (0 until array.length()).map { i ->
                        val d = array.getJSONObject(i)
                        DispositivoSalon(
                            id = d.optString("id"),
                            nombre = d.optString("nombre"),
                            idParticipante = d.opt("participante")
            ...
            "vista" -> {
                ...
                Mensaje.Vista(
                    VistaDelMando(
                        ...
                        contenidoPrivado = (0 until privado.length()).map { privado.getString(it) },
                        enunciado = o.optString("enunciado").takeIf {
